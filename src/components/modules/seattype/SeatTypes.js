...
const SeatTypes = (props) => {
  ...
  const { type_ID, name, price } = record;
    ...
    <Fragment>
      ...
      >
        <DialogTitle id='alert-dialog-title'>
          {current ? "Update Seat Type" : "Add Seat Type"}
        </DialogTitle>
        <form onSubmit={onSave}>
          <DialogContent>
            <DialogContentText id='alert-dialog-description'>
              <div className="row" style={{minWidth: '400px'}}> 
                <LabelContainer name='type_ID' value={type_ID} text='ID' />
                <InputContainer
                  id='name-inp'
                  type='text'
                  name='name'
...
const mapStateToProps = (state) => ({
  records: state.seattypes.records,
  current: state.seattypes.current,
  message: state.seattypes.message,
  error: state.seattypes.error,
});
...
export default connect(mapStateToProps, mapDispatchToProps)(SeatTypes);
