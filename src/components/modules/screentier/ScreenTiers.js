...
import {
  ...
  clearCurrentRecord,
  clearErrors,
} from "../../../actions/screentierActions";
...
const ScreenTiers = (props) => {
  ...
  const { tier_ID, price, description } = record;
    ...
    <Fragment>
      ...
      >
        <DialogTitle id='alert-dialog-title'>
          {current ? "Update Screen Tier" : "Add Screen Tier"}
        </DialogTitle>
        <form onSubmit={onSave}>
          <DialogContent>
            <DialogContentText id='alert-dialog-description'>
              <div className="row" style={{minWidth: '400px'}}> 
                <LabelContainer name='tier_ID' value={tier_ID} text='ID' />
                <InputContainer
                  id='price-inp'
                  type='text'
                  name='price'
...
const mapStateToProps = (state) => ({
  records: state.screentiers.records,
  current: state.screentiers.current,
  message: state.screentiers.message,
  error: state.screentiers.error,
});
...
export default connect(mapStateToProps, mapDispatchToProps)(ScreenTiers);
